...
export class HbdService {
    ...
    async sendQR(
        ...
        try {
            ...
            log.push(
                `Found ${users.length} users with birthdays in month: ${month}`,
            );
            data = users;
            ...
            if (Number(month) == 1) {
                ...
                if (bk && !bk.status) {
                    ...
                    log.push(
                        `Backup Google Sheet for year: ${year - 1} completed successfully`,
                    );
                }
                log.push(`================================================`);
            }
            ...
            if (insert) {
                ...
                data = res.data;
                log.push(
                    `Inserted ${res.data.length} users to Google Sheet successfully`,
                );
                log.push(`================================================`);
            }
            ...
            if (sendQR) {
                ...
                for (const user of data) {
                    log.push(
                        `${++count}. Sent QR Code to EMPNO: ${user.SEMPNO} started`,
                    );
            ...
            await this.mailService.sendMail({
                ...
                html: this.html(
                    `<h1>Log รายการส่งคูปองวันเกิดพนักงาน</h1><div>${logContent}</div>`,
                ),
            });
    ...
    async sendReport(dto: ExcelHbdDto, log?: string[]) {
        ...
        try {
            ...
            log?.push(
                `Retrieved ${res.data.length} records for month: ${month}, year: ${year}`,
            );
    ...
    async sendMailReport(
        ...
        <b>จดหมายอิเล็กทรอนิกส์ฉบับนี้ เป็นการส่งจากระบบอัตโนมัติ ไม่สามารถตอบกลับได้ หากท่านมีข้อสงสัยหรือต้องการสอบถามรายละเอียดเพิ่มเติม โปรดติดต่อผู้ดูแลระบบ โทร.2038 </b>`),
            ...
            log?.push(
                `Failed to send report to ${user.SEMPNO} ${name} (${email}): ${error.message}`,
            );
            throw new Error(
    ...
    async sendMailQR(user: any, log?: string[]) {
        ...
        const email =
            process.env.NODE_ENV != 'production'
                ...
                : user.MEMEML;
        try {
            const qrCode = await this.qrcodeService.generateDataURLWithCaption(
                key,
                `${user.SEMPNO} ${formatDate(user.BIRTHDAY, 'MMM')}'${String(new Date().getFullYear()).substring(2)}`,
                { width: 256 },
            );
            ...
            const sendmail = await this.mailService.sendMail({
                ...
                html: this.html(`<p>เรียน คุณ ${name}</p>
                    ...
                    <p>ในวันพิเศษนี้ ทาง บริษัทมิตซูบิชิ เอลเลเวเตอร์ เอเซีย จำกัด ขอส่งคูปองวันเกิดให้ท่านตามไฟล์แนบในอีเมลนี้</p>
                    <p><img src="cid:${cidName}" alt="Birthday QR Code" /></p>
                    <p>หมายเหตุ :</p>
                    <ul>
                        <li>แสดงคูปองนี้แด่ร้านค้าเพื่อแสกน</li>
                        <li>คูปองนี้มีมูลค่า 25 บาท (ส่วนที่เกินจากสิทธิ์ต้องชำระเองที่ร้านเท่านั้น)</li>
                        <li>สามารถใช้ที่ ร้าน Sleepless Café เท่านั้น</li>
                        <li>ใช้ได้ภายในเดือนเกิดของท่านครั้งเดียวเท่านั้น</li>
                        <li>สิทธิพิเศษนี้ไม่สามารถแลกเปลี่ยนเป็นเงินสดหรือของรางวัลอื่นๆ ได้</li>
                    </ul>
                    <b>จดหมายอิเล็กทรอนิกส์ฉบับนี้ เป็นการส่งจากระบบอัตโนมัติ ไม่สามารถตอบกลับได้</b> <br>
                    <b>หากท่านมีข้อสงสัยหรือต้องการสอบถามรายละเอียดเพิ่มเติม โปรดติดต่อฝ่ายทรัพยากรบุคคล  คุณจันทกานต์ เบอร์ต่อ 1163</b>`),
                attachments: [
                    {
            ...
            log?.push(
                `Failed to send email to ${user.SEMPNO} ${name} (${email}): ${error.message}`,
            );
            throw new Error(
