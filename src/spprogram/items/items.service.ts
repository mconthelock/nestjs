...
export class ItemsService {
    ...
    async findAll(query: searchItemsDto) {
        const qb = this.items
            ...
            .leftJoinAndSelect('itemcus.customer', 'customer');
        //.where('customer.CUS_STATUS = :status', { status: '1' });
    ...
    async validateDrawingNo(input) {
        ...
        for (const rule of lengthBasedRules) {
            if (
                dwg.length === rule.length &&
                /[G\-]/.test(dwg[rule.checkIndex])
            ) {
                return (
                    dwg.slice(0, rule.checkIndex) +
                    ' ' +
                    dwg.slice(rule.checkIndex)
                );
            }
        }
    ...
    async validateVariable(inputString) {
        ...
        if (matches.length === 0 && cleanedString.length > 0) {
            isValid = false;
            errors.push(
                'รูปแบบสตริงไม่ถูกต้อง: ไม่พบรูปแบบ KEY=VALUE ที่ถูกต้อง.',
            );
