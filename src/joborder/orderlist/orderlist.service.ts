...
export class OrderListService {
  ...
  async search(dto: SearchOrderListDto) {
    ...
    const {ORDTYPE, PRJ_NO, MFGNO, BUYEREMPNO, BUYERNAME, PRNO, PRDATE, PONO, PODATE, LINENO, VENDCODE, ITEM, DRAWING, INVOICE, DUEDATE, fields = []} = dto;
  ...
  async searchByPage(dto: SearchOrderListDto) {
    const { search = '', limit = 10, page = 1, fields = [], ORDTYPE, PRJ_NO, MFGNO, BUYEREMPNO, BUYERNAME, PRNO, PRDATE, PONO, PODATE, LINENO, VENDCODE, ITEM, DRAWING, INVOICE, DUEDATE } = dto;
    const query = this.OrderListRepository.createQueryBuilder('OrderList');
    ...
    if (fields.length > 0) {
        ...
        query.select(safeFields.map(f => `OrderList.${f}`));
        // if (search) {
        //     console.log('search fields:', safeFields);
        //     console.log('search', search);
        //     const [whereSql, whereParams] = getQueryFieldsBySelect(safeFields, this.numberFields, search);
    ...
    //     if (search) {
    //         console.log('search', search);
    //         const [whereSql, whereParams] = getQueryFieldsBySelect(this.allowFields, this.numberFields, search);
    //         // ส่งเข้า queryBuilder
    //         query.where(whereSql, whereParams);
    //     } 
    // }
    if (search) {
