...
export class OrderListService {
  ...
  async search(dto: SearchOrderListDto) {
    ...
    if (BUYERNAME)
      query.andWhere('OrderList.BUYERNAME LIKE :BUYERNAME', {
        BUYERNAME: `%${BUYERNAME}%`,
      });
    ...
    if (DRAWING)
      query.andWhere('OrderList.DRAWING LIKE :DRAWING', {
        DRAWING: `%${DRAWING}%`,
      });
    if (INVOICE)
      query.andWhere('OrderList.INVOICE LIKE :INVOICE', {
        INVOICE: `%${INVOICE}%`,
      });
  ...
  async serversite(dto: SearchOrderListDto) {
    ...
    const query = this.OrderListRepository.createQueryBuilder('OrderList');
    if (search) {
      query.andWhere('TO_CHAR(OrderList.PRNO) LIKE :search', {
        search: `%${search}%`,
      });
    }
