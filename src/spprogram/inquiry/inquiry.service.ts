...
export class InquiryService {
    ...
    async search(searchDto: searchInqDto) {
        ...
        if (searchDto.IS_DETAILS) {
            ...
            .leftJoinAndMapMany('inq.orders', Orderpart, 'orders', 'inq.INQ_NO = orders.INQUIRY_NO AND replace(details.INQD_DRAWING, \' \', \'\') = replace(orders.DWGNO_MELTEC, \' \', \'\') AND orders.REVISION_CODE != :rev', { rev: 'D' })
    ...
    async create(
        ...
        try {
            ...
            const detailPromises = details.map((el, d) => {
                ...
                if (item === 5) item = 2;
                if (item >= 6) item = 6;
                const grp_id_obj = savedGroups.find(
                    (val) => val.INQG_GROUP === item,
                );
        ...
        try {
            ...
            for (const dt of details) {
                ...
                if (!group) {
                    ...
                    const savedGroups = await runner.manager.findOne(
                        InquiryGroup,
                        {
                            where: {
                                INQ_ID: inquiry.INQ_ID,
            ...
            if (timelinedata !== undefined) {
                const timelineDto = plainToInstance(
                    updateTimelineDto,
                    timelinedata,
                );
