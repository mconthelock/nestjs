...
import {
  ...
  DeleteMatrixEffectItemDto,
  MigrationMatrixItemEffectDto,
} from './dto/create-matrix-effect-item.dto';
...
export class MatrixEffectItemService {
  ...
  async delete(dto: DeleteMatrixEffectItemDto, queryRunner?: QueryRunner) {
    let localRunner: QueryRunner | undefined;
    let didConnect = false;
    let didStartTx = false;
    try {
      ...
      const runner = queryRunner || localRunner!;
      await runner.manager.delete(MatrixEffectItem, dto);
      if (localRunner && didStartTx && runner.isTransactionActive)
        await localRunner.commitTransaction();
      return {
        status: true,
        message: 'Delete Matrix Effect Item Successfully',
      };
    } catch (error) {
      if (localRunner && didStartTx && localRunner.isTransactionActive)
        await localRunner.rollbackTransaction();
      throw new Error('Delete Matrix Effect Item Error: ' + error.message);
    } finally {
      if (localRunner && didConnect) await localRunner.release();
    }
  }
