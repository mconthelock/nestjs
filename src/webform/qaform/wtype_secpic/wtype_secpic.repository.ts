...
import { WTYPE_SECPIC } from 'src/webform/qaform/wtype_secpic/entities/wtype_secpic.entity';
...
export class WtypeSecpicRepository extends BaseRepository {
    ...
    async search(dto: FiltersDto) {
        const qb = this.manager.createQueryBuilder(WTYPE_SECPIC, 'WS');

        this.applyFilters(qb, 'WS', dto, ['TID', 'SECID']);
    ...
    findSectionsByWtype(tid: number) {
        return this.manager
            ...
            .innerJoin(SECPIC, 'S', 'S.SECID = W.SECID')
            .select('S.SEC', 'SEC')
