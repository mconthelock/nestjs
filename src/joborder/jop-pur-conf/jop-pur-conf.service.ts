...
export class JopPurConfService {
    ...
    async getRevisionHistoryByKey(
        list: { mfg: string; pono: number; line: number }[],
        queryRunner?: QueryRunner,
    ) {
        const repo = queryRunner
            ? queryRunner.manager.getRepository(JopPurConf)
            : this.confRepo;
        return await repo
            .createQueryBuilder('jop')
            .select([
                'jop.JOP_MFGNO AS MFGNO',
                'jop.JOP_PONO AS PONO',
                'jop.JOP_LINENO AS LINENO',
                'jop.JOP_REVISION AS REVISION',
                'jop.JOP_PUR_CONFIRM AS ACTION_BY',
                'jop.JOP_PUR_CONFIRM_DATE AS DDATE',
                'jop.JOP_PUR_REMARK AS REMARK',
                'jop.JOP_PUR_INPUT_DATE AS INPUT_DATE',
                'U.SNAME AS NAME',
                "'PUR' AS REVTYPE",
            ])
            .leftJoin('jop.purConfirm', 'U')
            .innerJoin(
                (qb) =>
                    qb
                        .select([
                            ...
                            'JOP_PONO',
                            'JOP_LINENO',
                            'MAX(JOP_REVISION) AS MAX_REV',
                        ])
                        .from('JOP_PUR_CONF', 'M')
                        .groupBy('JOP_MFGNO, JOP_PONO, JOP_LINENO'),
                'PUR',
                `jop.JOP_MFGNO = PUR.JOP_MFGNO
                  AND jop.JOP_PONO = PUR.JOP_PONO
                  AND jop.JOP_LINENO = PUR.JOP_LINENO`,
            )
            .where(
                new Brackets((qb) => {
                    list.forEach((item, i) => {
                        qb.orWhere(
                            `(jop.JOP_MFGNO = :mfg${i} AND jop.JOP_PONO = :pono${i} AND jop.JOP_LINENO = :line${i})`,
                            {
                                [`mfg${i}`]: item.mfg,
                                [`pono${i}`]: item.pono,
                                [`line${i}`]: item.line,
    ...
    async getRevisionHistory(
        ...
        if (queryRunner) {
            ...
            repo = this.confRepo;
            //   repo = this.dataSource.getRepository(JopPurConf);
        }
        return await repo
            .createQueryBuilder('jop')
    ...
    async search(dto: SearchJopPurConfDto, queryRunner?: QueryRunner) {
        ...
        const query = repo
            ...
            .innerJoin('(SELECT * FROM AMECUSERALL)', 'U', 'A.JOP_PUR_CONFIRM = U.SEMPNO');
        ...
        if (dto.JOP_LINENO)
            query.andWhere('A.JOP_LINENO = :lineno', {
                lineno: dto.JOP_LINENO,
            });
