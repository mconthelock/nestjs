...
dotenv.config();
let purConfig: TypeOrmModuleAsyncOptions;
if (process.env.HOST == 'AMEC') {
    purConfig = {
        ...
        useFactory: async (config: ConfigService, winstonLogger: Logger) => ({
            ...
            type: 'oracle',
            username: process.env.PUR_USER,
            password: process.env.PUR_PASSWORD,
            connectString: `${process.env.PUR_HOST}:${process.env.PUR_PORT}/${process.env.PUR_SERVICE}?expire_time=5`,
            entities: [
    ...
    purConfig = {
        ...
        useFactory: async (config: ConfigService, winstonLogger: Logger) => ({
            ...
            database: process.env.PUR_DATABASE,
            entities: [
                __dirname + '/../../**/**/*.entity{.ts,.js}',
                __dirname + '/../../**/**/**/*.entity{.ts,.js}',
            ],
