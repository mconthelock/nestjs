    ...
    async getRevisionHistoryByKey(
        ...
        return await repo
            ...
            .select([
                ...
                'jop.JOP_MAR_REQUEST AS ACTION_BY',
                'jop.JOP_MAR_REQUEST_DATE AS DDATE',
                'jop.JOP_MAR_REMARK AS REMARK',
                'jop.JOP_MAR_INPUT_DATE AS INPUT_DATE',
                'U.SNAME AS NAME',
                "'MAR' AS REVTYPE",
            ])
            .leftJoin('jop.marRequest', 'U')
            .innerJoin(
                (qb) =>
                    qb
                        .select([
                ...
                'MAR',
                `jop.JOP_MFGNO = MAR.JOP_MFGNO
                AND jop.JOP_PONO = MAR.JOP_PONO
                AND jop.JOP_LINENO = MAR.JOP_LINENO`,
            )
            .where(new Brackets(qb => {
                list.forEach((item, i) => {
                    qb.orWhere(
                        `(jop.JOP_MFGNO = :mfg${i} AND jop.JOP_PONO = :pono${i} AND jop.JOP_LINENO = :line${i})`,
                        { [`mfg${i}`]: item.mfg, [`pono${i}`]: item.pono, [`line${i}`]: item.line },
                    );
                });
            }))
