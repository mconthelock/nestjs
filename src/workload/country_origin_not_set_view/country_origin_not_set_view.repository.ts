...
@Injectable()
export class CountryOriginNotSetViewRepository extends BaseRepository {
    constructor(@InjectDataSource('workloadConnection') ds: DataSource) {
        super(ds); // นำค่าไปเก็บและใช้ใน BaseRepository
    }

    findAll() {
        return this.getRepository(COUNTRY_ORIGIN_NOT_SET_VIEW).find({
            order: { PLANNER: 'asc', PURCODE: 'asc' },
        });
    }
}
