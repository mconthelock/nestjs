...
export class ItemMfgService {
    ...
    async create(dto: CreateItemMfgDto) {
        try {
            dto.NQTY = !dto.NQTY ? null : dto.NQTY;
    ...
    async findOne(id: number) {
        try {
            ...
            throw new Error(
                `Search ITEM_MFG by id ${id} Error: ` + error.message,
            );
        }
    }
    ...
    async update(itemId: number, dto: UpdateItemMfgDto) {
        try {
            ...
            if (dto.NSTATUS != null) {
                ...
                const itemListRes =
                    await this.itemMfgListService.updateByItemId(itemId, data);
                ...
                const deleteRes =
                    await this.itemMfgDeleteService.updateByItemId(
                        itemId,
                        data,
                    );
                // update control drawing pis
                const controlRes =
                    await this.controlDrawingPisService.updateByItemId(
                        itemId,
                        data,
                    );
            }
    ...
    async remove(id: number) {
        try {
            ...
            throw new Error(
                `Delete ITEM_MFG by id ${id} Error: ` + error.message,
            );
        }
    }
}
