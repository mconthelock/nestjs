  ...
  async updateByItemId(ItemId: number, dto: UpdateControlDrawingPisDto) {
    try {
      ...
      if (res.affected === 0) {
        return {
          status: false,
          message: `Update CONTROL_DRAWING_PIS by ItemId ${ItemId} Failed`,
        };
      }
      return {
        status: true,
        message: `Update CONTROL_DRAWING_PIS by ItemId ${ItemId} Successfully`,
        data: res,
      };
    } catch (error) {
      throw new Error(
        `Update CONTROL_DRAWING_PIS by ItemId ${ItemId} Error: ` + error.message,
      );
    }
  }
