...
export class OrderListService {
    ...
    async orderlistNew(dto: SearchOrderListDto) {
        ...
        return await this.getRevisionHistoryByKey(data);
    }
    ...
    private setQueryNew(dto: SearchOrderListDto, type: string = 'data') {
        ...
        if (C_DBP)    query.andWhere('O.C_DBP = :C_DBP', { C_DBP });
        ...
        if (SREGISDATE && EREGISDATE) {
            query.andWhere(`O.REGISDATE >= TO_DATE(:SREGISDATE, 'YYYY-MM-DD') AND O.REGISDATE <= TO_DATE(:EREGISDATE, 'YYYY-MM-DD')` , { SREGISDATE, EREGISDATE });
        }
    ...
    async getRevisionHistoryByKey(list: any[]) {
        const map = new Map();
        const marConditions: { mfg: string; pono: number; line: number }[] = [];
        ...
        for (const l of list) {
            const marKey = `${'MAR'}_${l.MFGNO}_${l.PONO}_${l.LINENO}`;
            const purKey = `${'PUR'}_${l.MFGNO}_${l.PONO}_${l.LINENO}`;
            l.MAR_REVISION = map.get(marKey) || [];
            l.PUR_REVISION = map.get(purKey) || [];
        }
        return list;
    }
