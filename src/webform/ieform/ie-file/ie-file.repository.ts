...
import { IE_FILE } from 'src/common/Entities/webform/table/IE_FILE.entity';

@Injectable()
export class IeFileRepository extends BaseRepository {
    constructor(@InjectDataSource('webformConnection') ds: DataSource) {
        super(ds); // นำค่าไปเก็บและใช้ใน BaseRepository
    }

    async getFile(dto: SearchHandleFileFormDto) {
        return this.getRepository(IE_FILE).find({
            where: dto,
            order: {
                FILE_ID: 'ASC',
            },
    ...
    async create(dto: CreateHandleFileFormDto) {
        return await this.getRepository(IE_FILE).save(dto);
    }
}
