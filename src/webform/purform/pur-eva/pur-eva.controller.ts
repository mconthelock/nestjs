...
export class PurEvaController {
    constructor(
        private readonly purEvaService: PurEvaRequestService,
        private readonly purEvaUpdateService: PurEvaUpdateService,
        private readonly purEvaApproveService: PurEvaApproveService,
    ) {}
    ...
    @UseForceTransaction()
    approve(
        @Body() dto: ApprovePurevaFormDto, // หรือ RequestPurevaFormDto
        @Req() req: Request,
    ) {
        const ip = getClientIP(req);
        return this.purEvaApproveService.approve(dto, ip);
    }
}
